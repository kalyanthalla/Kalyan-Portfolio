...
const About = ({ isDarkMode }) => {
  return (
    <section 
      id='about' 
      className={`w-full px-4 sm:px-6 lg:px-8 pt-20 md:pt-24 pb-16 md:pb-20 min-h-screen flex items-center scroll-mt-20 transition-colors duration-300 relative overflow-hidden ${
      ...
      <div className="max-w-7xl mx-auto w-full relative z-10">
        <div className="w-full">
          ...
          <div className='flex flex-col lg:flex-row items-center justify-between gap-6 md:gap-8 lg:gap-12 w-full'>
            {/* Profile image */}
            <div className='relative order-1 lg:order-2 w-48 h-48 sm:w-56 sm:h-56 md:w-64 md:h-64 lg:w-72 lg:h-72 z-10 group mx-auto lg:mx-0'>
            ...
            <div className='flex-1 order-2 lg:order-1'>
              <div className="space-y-4 md:space-y-6">
                ...
                <div className={`p-4 sm:p-5 rounded-xl backdrop-blur-sm border ${
                  isDarkMode ? 'bg-neutral-800/50 border-neutral-700' : 'bg-white/80 border-neutral-200'
                } transition-all duration-300 hover:shadow-lg`}>
                  <h3 className={`text-sm sm:text-base font-semibold poppins mb-2 sm:mb-3 flex items-center gap-2 ${
                    isDarkMode ? 'text-purple-400' : 'text-purple-600'
                  }`}>
                  ...
                  <ul className="space-y-1 sm:space-y-2 pl-5">
                    {[
                      "Creating elegant solutions to complex problems",
                      "Continuous learning and staying updated",
                      "Building applications that make a difference"
                    ].map((item, index) => (
                      <li key={index} className={`text-xs sm:text-sm relative pl-4 before:absolute before:left-0 before:top-2 before:w-1.5 before:h-1.5 before:rounded-full ${
